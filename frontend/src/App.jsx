  ...
  return (
    <div className="h-screen w-full bg-[#0C0C0C]">
      ...
      <header className="w-full h-[8vh] md:h-[10vh] flex items-center gap-[1.2rem] px-[2.8vw] md:px-[1vw] bg-[#080808] border border-b-white/60">
        <img className="w-[2.3em] h-[2.3em] md:w-16 md:h-16 rounded-full object-center object-cover" src={aiImg} alt="" />
      ...
      {/* mid */}
      { messages.length === 0 ? 
        <section className='h-[84vh] md:h-[77vh] font-mono text-gray-300 text-sm md:text-[1.2em] w-full flex flex-col gap-[0.5em] items-center justify-center font-["ArboriaBook"]'>
          <img className='w-[10em] h-[10em] rounded-full mb-[1em] object-cover drop-shadow-lg drop-shadow-blue-500/80' src={aiImg} alt="" />
           <TypeAnimation
  sequence={[
    "Hi, I'm Xoni.",
    1000,
...
{showThird && (
  ...
  />
        ...
        {messages.map((msg,idx)=>{
          ...
          return <div key={idx} className='text-white w-full mb-[0.5em]'>
              {msg.role === 'user' ? <UserChat msg={msg.parts}/> : <AiChat msg={msg.parts}/>}
          </div>
        })}
      </section>
      }
      ...
      <section className='w-full h-[8vh] md:h-[13vh] flex items-center gap-[1em] md:gap-[1.2vw] px-[2.8vw] md:px-[1vw] font-["ArboriaBook"]'>
        ...
        type="text" />
        <button onClick={(e)=>{SendHandler(e)}} className='w-[2.1em] h-[2.1em] md:w-12 md:h-12 flex items-center justify-center rounded-full bg-[#673bf6]'>
